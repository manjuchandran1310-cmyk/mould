...
export default function Page() {
  ...
  const modelResult = goal === 'ui' ? 'Product preview: the visible screen reads the output and turns it into a user-facing state.' : goal === 'mechanics' ? `${detail.label} is the exact operation at line ${selectedLine}; inspect its inputs, return value, and side effects.` : modelOutput
  ...
  function analyze() { setAnalyzed(false); setIsModelRunning(true); setActiveStep(0); let step = 0; const timer = window.setInterval(() => { step += 1; setActiveStep(step % modelSteps.length); if (step >= modelSteps.length) { window.clearInterval(timer); setAnalyzed(true); setIsModelRunning(false); setModelOutput(goal === 'story' ? `The program reads ${modelInput || 'the input'}, transforms it, and returns a result another screen can show.` : goal === 'trace' ? `Input → line ${selectedLine} → transformed value → returned output. The highlighted line is the current hand-off.` : goal === 'change' ? 'Changing the input changes the path: test empty, large, and unexpected values before shipping.' : goal === 'break' ? 'Try an empty list, missing property, or failed request. These are the assumptions the current code does not guard yet.' : modelResult) } }, 350) }
