...
export default function Page() {
  ...
  function analyze() { if (!hasUserInput) { setModelOutput('Paste a real input used by this code before running it.'); return } setAnalyzed(false); setIsModelRunning(true); setActiveStep(0); let step = 0; const timer = window.setInterval(() => { step += 1; setActiveStep(step % modelSteps.length); if (step >= modelSteps.length) { window.clearInterval(timer); setAnalyzed(true); setIsModelRunning(false); setModelOutput(`${goalResult} Input used: ${modelInput}`) } }, 350) }
  ...
  function reset() { setCode(sampleCode); setSelectedLine(8); setSelectedRange([8, 8]); setTool('select'); setThreads([]); setNote(''); setMarks([]); setStrokes([]); setDrawingStroke(null); setAnalyzed(true); setPointing(false); setPointReady(false); setPointStart(null); setActiveStep(0); setIsPlaying(false); setContextAction(null); setFeedback('unreviewed') }
