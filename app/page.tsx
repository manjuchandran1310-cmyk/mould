...
export default function Page() {
  ...
  return <main className="mould-app">
    ...
    : <section className="playground"><div className="playground-intro"><span className="pane-label">Playground · design ↔ engineering</span><h2>Turn a fuzzy product idea into a better handoff.</h2><p>Designers can ask what the code needs. Engineers can answer with constraints, examples, and a small next step — without making either person feel like they should already know the other discipline.</p></div><div className="playground-grid"><div className="play-card"><span className="step-num">01</span><h3>Bring a feature</h3><p>“A user should be able to save a favorite.”</p><button onClick={()=>setQuestion('What does engineering need from this feature?')}>Ask what engineering needs <ArrowRight/></button></div><div className="play-card"><span className="step-num">02</span><h3>See the merge point</h3><p>Product intent → UI state → data shape → edge cases.</p><button onClick={()=>setQuestion('Where does design and engineering meet here?')}>Show the shared decision <ArrowRight/></button></div><div className="play-card"><span className="step-num">03</span><h3>Make the handoff kinder</h3><p>Leave one clear question, one acceptance example, and one known trade-off.</p><button onClick={()=>setQuestion('What should I clarify before handing this off?')}>Draft a handoff question <ArrowRight/></button></div></div><div className="handoff"><div><span className="pane-label">A shared language</span><p><b>Designer:</b> “What should happen when the list is empty?”</p><p><b>Engineer:</b> “We need an empty state and a loading state. Let’s agree on the copy before I build it.”</p></div><div className="handoff-arrow">→</div><div><span className="pane-label">The better outcome</span><p>A decision both people can test, explain, and change together.</p></div></div></section>}
